...
// #define MATRIX_M 9600
// #define MATRIX_N 9600
// #define MATRIX_K 9600
#define MATRIX_M 32
#define MATRIX_N 32
#define MATRIX_K 32
// #define MATRIX_M 128
...
// __global__ void simtNaiveKernel(const half *__restrict__ A, const half *__restrict__ B, float *__restrict__ C, size_t M,
//                                 size_t N, size_t K) {
...
//     simtNaiveKernel<<<grid, block,0,streams[0]>>>(A, B, C, M, N, K);
// }
...
__global__ void wmmaNaiveKernel(const half *__restrict__ A, const half *__restrict__ B, float *__restrict__ C, size_t M,
    ...
    if (threadIdx.x % warpSize == 0 ) {
        printf("wmmaNaiveKernel: warp_row = %d, warp_col = %d\n", warp_row, warp_col);
    }
    ...
    //! bound
    if (warp_row >= THRESHOLD_ROW || warp_col >= THRESHOLD_COL) {
        return;
    }
...
#pragma unroll
    ...
    // Conditionally store results only if within the computed dimensions and outside the thresholded area
    if ((warp_row < THRESHOLD_ROW || warp_col < THRESHOLD_COL) && (warp_row < M && warp_col < N)) {
...
// }
__global__ void sp_example(half *__restrict__ a, half * __restrict__ b, float * __restrict__ c, int M, int N, int K) {
  ...
  for (int k = 0; k < K; ++k) {
    ...
    // add without alpha and beta
    // sum += a_val + b_val;
    sum = __hadd(sum, __hadd(a_val,b_val));
...
// simtNaiveKernel<<<grid, block,0,streams[0]>>>(a_fp16, b_fp16, d_C, MATRIX_M, MATRIX_N, MATRIX_K);
// simtNaive(a_fp16, b_fp16, d_C, MATRIX_M, MATRIX_N, MATRIX_K);

dim3 block(32);
dim3 grid(div_ceil(N, WMMA_N), div_ceil(N, WMMA_M));

// wmmaNaiveKernel<<<grid, block,0,0>>>(a_fp16, b_fp16, d_C, MATRIX_M, MATRIX_N, MATRIX_K);
wmmaNaiveKernel<<<grid, block,0,streams[0]>>>(a_fp16, b_fp16, d_C, MATRIX_M, MATRIX_N, MATRIX_K);
// wmmaNaive(a_fp16, b_fp16, d_C, MATRIX_M, MATRIX_N, MATRIX_K);
...
//       a_fp16, b_fp16, d_C, MATRIX_M, MATRIX_N, MATRIX_K, alpha, beta);
  ...
  sp_example<<<sp_gridDim, sp_blockDim, 0,  streams[1]>>>(
      a_fp16, b_fp16, d_C, MATRIX_M, MATRIX_N, MATRIX_K);

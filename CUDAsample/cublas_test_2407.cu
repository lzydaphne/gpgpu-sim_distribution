...
void test_gemm(cublasHandle_t handle, int m, int n, int k, T *A, T *B, S *C,
               S *alpha, S *beta, int algo, int iteration, int id,cudaStream_t stream) {
    float total_time = 0;
...
int main() {
    ...
    // allocate_memory(m, n, k, &hA3, &hB3, &hC3);
    ...
    for (int i = 0; i < m * k; ++i) {
        ...
        // hA3[i] = __float2half_rn(fA2[i]);
        // iA[i] = float2int8(fA[i], 127);
    } 
    for (int i = 0; i < k * n; ++i) {
        fB2[i] = float(i % 255 - 127) / 127;
        hB2[i] = __float2half_rn(fB2[i]);
        // hB3[i] = __float2half_rn(fB2[i]);
        // iB[i] = float2int8(fB[i], 127);
    }
    ...
    // test_gemm(handle, m, n, k, hA2, hB2, hC2, &f_alpha, &f_beta, 103, iteration, 1, stream2);
    // cublasSetStream(handle, stream1);
    // test_gemm(handle, m, n, k, hA, hB, hC, &f_alpha, &f_beta, 5, iteration,1, stream1);
    // cublasSetStream(handle, stream3);
    // test_gemm(handle, m, n, k, hA3, hB3, hC3, &f_alpha, &f_beta, 103, iteration, 1,stream3);

    //! sequential
    test_gemm(handle, m, n, k, hA2, hB2, hC2, &f_alpha, &f_beta, 103, iteration, 1,0);
    // test_gemm(handle, m, n, k, hA, hB, hC, &f_alpha, &f_beta, 5, iteration, 1,0);
    // test_gemm(handle, m, n, k, hA3, hB3, hC3, &f_alpha, &f_beta, 103, iteration, 1,0);
    //! sp
